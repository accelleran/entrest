...
- The default maximum number of results that can be eager-loaded for a given edge can be controlled
  with the [`EagerLoadLimit`](/entrest/openapi-specs/configuration/#eagerloadlimit) config option
...
- If you eager load an edge, by default the associated edge endpoint will have its pagination turned off.
  This can be turned off with the [`DisableEagerLoadNonPagedOpt`](/entrest/openapi-specs/configuration/#disableeagerloadnonpagedopt) config option.
- If you only ever want to eager load a specific edge and not have a dedicated endpoint, you can use
  ...
  to disable the edge endpoint. This can also be configured globally with the [`DisableEagerLoadedEndpoints`](/entrest/openapi-specs/configuration/#disableeagerloadedendpoints) config option.
- All edges can be eager loaded by default (though highly discouraged). See the [`DefaultEagerLoad`](/entrest/openapi-specs/configuration/#defaulteagerload) config option.
...
via the [`EagerLoadLimit`](/entrest/openapi-specs/configuration/#eagerloadlimit) config option
and associated [`WithEagerLoadLimit`](/entrest/openapi-specs/annotation-reference/#witheagerloadlimit)
annotation.
